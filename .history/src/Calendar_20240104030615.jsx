...
export const Calendar = () => {
  ...
  const getTimes = () => {
    ...
    const { justDate } = date;
    const selectedDay = daysArray.find(day => day.date.getTime() === justDate.getTime());
    console.log(selectedDay)

    const beginningMorning = add(justDate, { hours: selectedDay.morning.openingTime });
    const endMorning = add(justDate, { hours: selectedDay.morning.closingTime });
    const beginningAfternoon = add(justDate, { hours: selectedDay.afternoon.openingTime });
    const endAfternoon = add(justDate, { hours: selectedDay.afternoon.closingTime });
    const interval = INTERVAL;

    const times = [];
    for (let i = beginningMorning; i <= endMorning; i = add(i, { minutes: interval })) {
      times.push(new Date(i));
    }
  
    // Generate times for the afternoon
    for (let i = beginningAfternoon; i <= endAfternoon; i = add(i, { minutes: interval })) {
      times.push(new Date(i));
    }
    return times;
  };
  ...
  const handleCalendarClose = () => {
    // Reset date and hide the calendar
    setDate((prevDate) => ({ ...prevDate, justDate: undefined }));
  ...
  return (
    <div className="bg-slate-600 flex w-screen gap-6 justify-center">
      ...
      <div className=" bg-blue-600" onClick={() => console.log(getDaysArray())}> Click here to display all days</div>
