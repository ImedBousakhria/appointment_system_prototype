...
const TimeSelection = ({ allTimes, activeTime, onClick, onBack }) => (
  ...
  >
    <div className="mb-3 border-b pb-2 flex justify-between items-center px-1">
      ...
      <span onClick={onBack} className="border p-1.5 cursor-pointer hover:bg-gray-300">
        Back
      </span>
    </div>
    <FixedSizeList
...
const DateSelection = ({ onSelectDate, onSelectTime }) => (
  ...
  >
    ...
    />
    <button onClick={onSelectTime} className="mt-3 w-full bg-blue-500 text-white p-2 rounded-md">
      Select a Time Slot
    </button>
  </motion.div>
);
...
export const Calendar = () => {
  ...
  const handleDateClick = (selectedDate) => {
    ...
    setShowCalendar(true); // Show the calendar after selecting a date
  };
  ...
  const allTimes = getTimes(
    OPENING_TIME_MORNING,
    CLOSING_TIME_MORNING
  ).concat(getTimes(OPENING_TIME_AFTERNOON, CLOSING_TIME_AFTERNOON));
