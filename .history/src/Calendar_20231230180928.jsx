...
export const Calendar = () => {
  const [date, setDate] = useState({ justDate: undefined, dateTime: undefined });
  ...
  return (
    <div className=" bg-slate-600 h-[400px] flex gap-6 items-center justify-center">
      <ReactCalendar
        minDate={new Date()}
      ...
      />
      {date.justDate ? (
        <div className=" bg-white h-[100%] overflow-y-auto  flex flex-col gap-3">
          {allTimes?.map((time, i) => (
            <div
              key={`time-${i}`}
